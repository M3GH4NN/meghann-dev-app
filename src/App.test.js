...
test("renders updated current resume headline and role summary", () => {
  ...
  expect(screen.getByText(/Frontera Software’s AWS infrastructure/i)).toBeInTheDocument();
});

test("renders corrected work timeline dates and current DoE roles", () => {
  render(<App />);

  expect(within(getWorkItem("Convo Communications")).getByText(/DevOps Lead/i)).toBeInTheDocument();
  expect(within(getWorkItem("Convo Communications")).getByText(/Jun 17 2024 to Present/i)).toBeInTheDocument();

  expect(within(getWorkItem("ReefMind LLC")).getByText(/Director of Engineering/i)).toBeInTheDocument();
  expect(within(getWorkItem("ReefMind LLC")).getByText(/May 22 2026 to Present/i)).toBeInTheDocument();

  expect(within(getWorkItem("Miller Mind Solutions Inc.")).getByText(/Director of Engineering/i)).toBeInTheDocument();
  expect(within(getWorkItem("Miller Mind Solutions Inc.")).getByText(/May 22 2026 to Present/i)).toBeInTheDocument();
  ...
  expect(
    within(getWorkItem("Frontera Software")).getByText(/Maintain and support Frontera's AWS infrastructure/i)
  ).toBeInTheDocument();

  expect(within(getWorkItem("EITR Technologies, LLC")).getByText(/Platform Engineer/i)).toBeInTheDocument();
  expect(within(getWorkItem("EITR Technologies, LLC")).getByText(/Dec 2022 to May 31, 2024/i)).toBeInTheDocument();
});
